...
class DreView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Calculate month numbers for the next 12 months for chart
        meses_chart = [(current_month + i - 1) % 12 + 1 for i in range(12)]

        # Calculate month numbers for next 6 months for table
        meses_table = [(current_month + i - 1) % 12 + 1 for i in range(6)]

        ordem_categorias = {'Receitas': 1, 'Investimentos': 2, 'Gastos Fixos': 3, 'Gastos Variáveis': 4}
        categorias = sorted(Categoria.objects.all(), key=lambda x: ordem_categorias[x.nome])
        
        resumo_chart = {}
        resumo_table = {}
        
        # Process transactions for chart and table separately
        for meses, resumo in [(meses_chart, resumo_chart), (meses_table, resumo_table)]:
            for categoria in categorias:
                resumo[categoria.nome] = {'subcategorias': {}, 'total': [0]*len(meses)}
                for subcategoria in Subcategoria.objects.filter(categoria=categoria):
                    ...
                    resumo[categoria.nome]['subcategorias'][subcategoria.nome] = [0]*len(meses)
            ...
            # Calculate the net total outside the loop
            resumo['Liquido'] = {'total': [0]*len(meses)}
            for mes in range(len(meses)):
                receita = resumo.get('Receitas', {'total': [0]*len(meses)})['total'][mes] or 0
                gastos_fixos = resumo.get('Gastos Fixos', {'total': [0]*len(meses)})['total'][mes] or 0
                gastos_variaveis = resumo.get('Gastos Variáveis', {'total': [0]*len(meses)})['total'][mes] or 0
                investimentos = resumo.get('Investimentos', {'total': [0]*len(meses)})['total'][mes] or 0
                resumo['Liquido']['total'][mes] = receita - (gastos_fixos + gastos_variaveis + investimentos)

        nomes_meses = ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro']
        nomes_meses_chart = [nomes_meses[mes-1] for mes in meses_chart]
        nomes_meses_table = [nomes_meses[mes-1] for mes in meses_table]
        context['resumo'] = resumo_table
        context['meses'] = list(zip(meses_table, nomes_meses_table))
        context['resumo_chart'] = resumo_chart
        context['meses_chart'] = list(zip(meses_chart, nomes_meses_chart))
        return context
